...
void test3(ostream &fp){
	...
	for(int i=1; i<200; i++) {
		...
		for(int j=50*i; j<51*i; j++) 
			if(D[j] != -10.0 && i % 39 == 0){
				fp << "D["<<j<<"] is : "<<D[j] << endl;  //should output where D[j] is : j once per value of i
				initCounter++;
			} 
	}
	int printNumber = 0;
	...
	for(int i =0; i < printNumber; i++) fp << "This print counts as a line" << endl;
	CircularDynamicArray<char> E(1000000);
	for(int i = 0; i < 1000000; i++) E[i] = 'X';
	E[490000] = 'A';
	E[500000] = 'B';
	fp << E.linearSearch('B')<< endl;   //should output 500000
	if(printWrapper) fp << "-----------------Finished Test----------------------" << endl;
}
...
void test6(ostream &fp){
	...
	for(int i = 0; i < 40; i++)
	{
		int value = A.binSearch(i); //FIXME HARDER DADDY
